...
| Nothing governing states what a terminal node announces. | VERIFIED | `fb.workflow::CONSTITUTION_WORKFLOW_V0` governs the act and does not use the word. `fb.event::CONSTITUTION_EVENT_V0` speaks of records being immutable once written and says nothing about how many an act may write. `fb.event::INVARIANT_EV_APPEND_ONLY_V0` and `fb.event::INVARIANT_EV_SCHEMA_REQUIRED_V0` require append-only writing and a declared schema, and neither counts. | S1 system_beliefs #3 |
...
|-----------|------|--------------|--------------------------------|-----------|
| Governs the act | fb.workflow::CONSTITUTION_WORKFLOW_V0 | Declares what an act is, how it routes, and where it ends. | PARTIAL | Says nothing about what a terminal node announces, or how many moments it may. |
| Governs a moment | fb.event::CONSTITUTION_EVENT_V0 | Declares what a moment is and that its record is immutable once written. | PARTIAL | Says nothing about how many moments one act may announce. |
| Holds a moment's record append-only | fb.event::INVARIANT_EV_APPEND_ONLY_V0 | Refuses a moment's record being altered after it is written. | EXACT | Nothing — it is why an announced moment cannot be unannounced. |
| Requires a moment to declare its schema | fb.event::INVARIANT_EV_SCHEMA_REQUIRED_V0 | Refuses a moment with no declared shape. | EXACT | Does not count announcements. |
