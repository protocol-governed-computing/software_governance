...
| Nothing governing states that an act resolves its records against one description, or why it should. | VERIFIED | fb.runtime_binding::CONSTITUTION_RUNTIME_BINDING_V0 governs bindings and never mentions where storage is described. Its four invariants — fb.runtime_binding::INVARIANT_RB_BINDING_POLICY_CONFORMANCE_V0, fb.runtime_binding::INVARIANT_RB_CS_ONLY_V0, fb.runtime_binding::INVARIANT_RB_NO_LOGIC_V0 and fb.runtime_binding::INVARIANT_RB_PARAMETERS_DECLARED_V0 — mention it in no form. The singular exists only as a field's declared type. | S1 system_beliefs #3 |
...
|-----------|------|--------------|--------------------------------|-----------|
| Governs bindings | fb.runtime_binding::CONSTITUTION_RUNTIME_BINDING_V0 | Declares what a binding is and what it may carry. | PARTIAL | Says nothing about where storage is described, or how much of it an act reaches. |
| Holds a binding to its declared parameters | fb.runtime_binding::INVARIANT_RB_BINDING_POLICY_CONFORMANCE_V0 | Checks the binding policy the compiler sealed. | PARTIAL | Checks the one description it finds, and cannot ask whether there should be more. |
